...
export default function UrlList({ 
  ...
  domains = ['linkly.to'], setDomains, settings = { redirectType: 302, shortCodeLength: 6 }, setSettings 
}) {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '8px' }}>
            ...
            {domains.map((d) => (
              <div key={d} style={{ background: 'var(--bg-color)', border: '1px solid var(--card-border)', borderRadius: '8px', padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontFamily: 'monospace', fontWeight: 600 }}>{d} {d === 'linkly.to' && '(Default)'}</span>
                {d === 'linkly.to' ? (
